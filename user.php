...
class User {
  ...
  public function checkPassword($currentPassword){
    $stmt = $this->pdo->prepare("SELECT * FROM users WHERE password = :password");
    $stmt->bindValue(':password', $currentPassword, PDO::PARAM_STR);
    $stmt->execute();
    return $stmt->fetch(PDO::FETCH_ASSOC);
  }

  public function passwordReset($newPassword){
